...
fn bellman_kalaba_max_road(graph: &Vec<Vec<i64>>) -> i64 {
    let mut last_line = vec![0; graph.len()];

    for i in 1..graph.len() {
        last_line[i] = graph[i][0];
    }

    loop {
        let current_line = once(0)
            .chain((1..graph.len()).map(|i| {
                (0..graph.len())
                    .filter(|&j| j != i)
                    .map(|j| graph[i][j] + last_line[j])
                    .max()
                    .unwrap_or(0)
            }))
            .collect_vec();
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let result = bellman_kalaba_max_road(&simplified);
    println!("{result}");

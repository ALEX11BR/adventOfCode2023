...
fn main() {
    ...
    let result = (0..layout.len())
        ...
        .chain(
            (0..layout.len())
                .map(|i| {
                    ((i as i32, layout[0].len() as i32 - 1), (0, -1))
                })
        )
        .chain(
            (0..layout.len())
        ...
        .chain(
            (0..layout.len())
                .map(|j| {
                    ((layout.len() as i32 - 1, j as i32), (-1, 0))
                })
        )
        .map(|(position, direction)| {
            let mut energized = vec![vec![0; layout[0].len()]; layout.len()];

            walk_light(&layout, &mut energized, position, direction);
            energized.iter().map(|l| l.iter()).flatten().map(|e| if *e > 0 {1} else {0}).sum()
        })
        .max()
        .unwrap_or(0);
